...
The ChatGPT fallback fills the prompt composer. Automatic submission is disabled by default and is controlled by the user in the options page. Users can separately control whether ChatGPT opens when a Forvo page opens. ChatGPT tab reuse is scoped to the configured ChatGPT URL, so unrelated ChatGPT conversations are ignored. If an existing configured ChatGPT tab was open before extension reload, the extension may attach its packaged content script to that tab without reloading the chat.
